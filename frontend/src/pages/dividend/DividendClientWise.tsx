/**
 * Dividend Payable - Client-wise View
...
interface DividendClientWiseData {
  company: string;
  company_id?: number;
  boid: string;
  client_name: string;
...
const DividendClientWise: React.FC = () => {
  // ============================================================================
  // STATE MANAGEMENT
  // ============================================================================
  ...
  // Fetch dividend payables
  const { data: dividendData, isLoading: isLoadingDividend, isError: isErrorDividend } = useDividendPayables();
  ...
  const processedData: DividendClientWiseData[] = useMemo(() => {
    ...
    const aggMap = new Map<string, DividendClientWiseData>();
  ...
  }, [dividendData, dateRange, selectedCompany]);

  // ============================================================================
  // COLUMN DEFINITIONS
  // ============================================================================

  const columns: ColumnDef<DividendClientWiseData>[] = [
    {
      accessorKey: 'company',
      header: 'Company',
      size: 200,
  ...
  const handleExport = async () => {
    try {
      ...
      link.setAttribute(
        'download',
        `dividend-clientwise-${new Date().toISOString().split('T')[0]}.xlsx`
      );
  ...
  if (isLoadingDividend || isLoadingCompanies) {
    return (
      <>
        <NavigationBar />
        ...
        <div className="dashboard-container">
          <Container fluid>
            <Alert variant="danger" className="mt-4">
              <Alert.Heading>Error Loading Dividend Data</Alert.Heading>
              <p className="mb-0">Failed to load dividend payables. Please try again.</p>
            </Alert>
          </Container>
        </div>
      </>
      ...
      <div className="dashboard-container">
        <Container fluid>
          ...
          <h2 className="dashboard-header">
            <FaUsers style={{ marginRight: '0.5rem' }} /> Dividend Payable - Client-wise View
          </h2>
