/**
 * Dividend Payable - Summary Reports
 * Phase 2 Optimized
 */
...
const DividendSummaryReports: React.FC = () => {
  ...
  }, [dividendData, dateRange, viewType, companiesData]);
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await api.get('/dividend-payables/summary/', { params, responseType: 'blob' });

      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `dividend-summary-${new Date().toISOString().split('T')[0]}.xlsx`);
      ...
      <div className="dashboard-container">
        <Container fluid>
          <h2 className="dashboard-header">
            <FaChartPie style={{ marginRight: '0.5rem' }} /> Dividend Payable - Summary Reports
          </h2>
              ...
              <Row className="g-3">
                ...
                <Col md={6}>
                  ...
                  <ButtonGroup className="d-block w-100">
                    <Button
                      variant={viewType === 'sector' ? 'success' : 'outline-secondary'}
                      onClick={() => setViewType('sector')}
                    >
                      Sector
                    </Button>
                    <Button
                      variant={viewType === 'company' ? 'success' : 'outline-secondary'}
                      onClick={() => setViewType('company')}
                    >
                      Company
                    </Button>
                    <Button
                      variant={viewType === 'status' ? 'success' : 'outline-secondary'}
